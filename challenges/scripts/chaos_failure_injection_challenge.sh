...
# for Observability per CONST-035 + CONST-050(B). Cascade per CONST-051(A).

set -uo pipefail
HEALTH_URL="${OBSERVABILITY_HEALTH_URL:-}"
CHAOS_HOST="${OBSERVABILITY_CHAOS_HOST:-localhost}"
CHAOS_PORT="${OBSERVABILITY_CHAOS_PORT:-}"
LEGIT_REQS="${CHAOS_LEGIT_REQUESTS:-100}"
MIN_PCT="${CHAOS_LEGIT_MIN_PASS_PCT:-95}"

echo "=== Observability Chaos Failure-Injection Challenge ==="
echo "  url=$HEALTH_URL host=${CHAOS_HOST}:${CHAOS_PORT}"

if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: OBSERVABILITY_HEALTH_URL/_PORT unset — SKIP-OK: #env-no-target"
    echo "=== Observability Chaos Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability Chaos Challenge: PASSED ==="
