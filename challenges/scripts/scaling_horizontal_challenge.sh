...
# Observability per CONST-035 + CONST-050(B). Cascade per CONST-051(A).

set -uo pipefail
REPLICAS="${OBSERVABILITY_SCALING_REPLICA_URLS:-}"
REQS="${SCALING_REQS_PER_REPLICA:-50}"
CONC="${SCALING_CONCURRENCY:-10}"
MIN_PCT="${SCALING_MIN_PASS_PCT:-95}"

echo "=== Observability Scaling Challenge ==="
echo "  replicas=$REPLICAS reqs=$REQS conc=$CONC pass≥${MIN_PCT}%"

if [[ -z "$REPLICAS" ]]; then
    echo "[1/6] SKIP: OBSERVABILITY_SCALING_REPLICA_URLS unset — SKIP-OK: #env-single-replica"
    echo "=== Observability Scaling Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability Scaling Challenge: PASSED ==="
