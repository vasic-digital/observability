...
# Observability per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $OBSERVABILITY_HEALTH_URL.
...
echo "=== Observability DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: OBSERVABILITY_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Observability DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability DDoS Challenge: PASSED ==="
