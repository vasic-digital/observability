...
set -uo pipefail
BIN_PATH="${OBSERVABILITY_BIN:-}"
TIMEOUT_SEC="${UI_TIMEOUT_SEC:-30}"
USER_HOSTILE=('panic:' 'goroutine [0-9]+ \[running\]:' 'runtime error:' 'segmentation fault' 'fatal error:')

echo "=== Observability UI Terminal-Interaction Challenge ==="
echo "  bin=$BIN_PATH timeout=${TIMEOUT_SEC}s"

if [[ -z "$BIN_PATH" ]] || [[ ! -x "$BIN_PATH" ]]; then
    echo "[1/4] SKIP: OBSERVABILITY_BIN not set — SKIP-OK: #env-binary-missing"
    echo "=== Observability UI Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability UI Challenge: PASSED ==="
