...
echo "=== Observability UX End-to-End Flow Challenge ==="
echo "  bin=$BIN_PATH timeout=${TIMEOUT_SEC}s"

if [[ -z "$BIN_PATH" ]] || [[ ! -x "$BIN_PATH" ]]; then
    echo "[1/5] SKIP: OBSERVABILITY_BIN unset — SKIP-OK: #env-binary-missing"
    echo "=== Observability UX Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability UX Challenge: PASSED ==="
