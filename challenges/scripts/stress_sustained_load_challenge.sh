...
echo "=== Observability Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: OBSERVABILITY_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Observability Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Observability Stress Challenge: PASSED ==="
